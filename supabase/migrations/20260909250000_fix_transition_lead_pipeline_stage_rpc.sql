-- Migration: 20260909250000_fix_transition_lead_pipeline_stage_rpc.sql
-- Descrição: Remover last_interaction_at = clock_timestamp() de transition_lead_pipeline_stage RPC para preservar a semântica de interação comercial.
...
BEGIN
  ...
  -- 6. Transição atómica: Atualizar o lead (SEM alterar last_interaction_at)
  UPDATE public.leads
...
COMMENT ON FUNCTION public.transition_lead_pipeline_stage(UUID, TEXT, TEXT, UUID) IS
  'Função PL/pgSQL transacional atómica para alteração de pipeline_stage com locking FOR UPDATE e registo em pipeline_stage_history (sem alterar last_interaction_at).';
