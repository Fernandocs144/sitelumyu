...
export async function handleGetConversationsRequest(request) {
  ...
  const params = parseConversationsQueryParams(url.searchParams);

  // 4. Executar consulta à base de dados
  try {
    const result = await fetchAdminConversationsFromDatabase(serviceClient, params);
    ...
    return createAdminJsonResponse(
      { ok: false, error: 'Erro ao obter a listagem de conversas' },
      500,
      session.newCookies
    );
  }
