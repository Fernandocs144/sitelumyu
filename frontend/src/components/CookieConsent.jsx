...
export default function CookieConsent() {
  ...
  return (
    ...
    >
      <h2 id="lumyo-cookie-title" className="font-display text-lg tracking-wide sm:text-xl">
        {copy.title}
      </h2>
      <p id="lumyo-cookie-description" className="mt-2 font-body text-sm leading-relaxed text-white/75">
        {copy.description}{' '}
        <Link to="/cookies" className="underline decoration-magenta underline-offset-4 hover:text-white">
          {copy.learnMore}
        </Link>
      </p>

      {preferencesOpen && consent !== null && (
        <p className="mt-2 font-body text-xs text-white/55">{copy.currentChoice}</p>
      )}
